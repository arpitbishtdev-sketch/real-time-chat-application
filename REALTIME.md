...
Implemented as Socket.IO middleware (`io.use((socket, next) => {...})`), run once per connection attempt, **before** any event handlers are registered for that socket:

1. Read the JWT from the handshake's cookie header (same `accessToken` cookie REST uses — no separate socket-specific token).
2. Verify signature + expiry. Failure → `next(new Error("unauthorized"))`, which rejects the handshake (client receives a `connect_error`).
...
- Joining `conversation:<id>` (on `conversation:join`): reject if not a participant.
- Sending a message (`message:send`): reject if not a participant of the target `conversationId`.
- Read receipts / typing events: same check.
...
|---|---|---|---|---|---|---|
| `conversation:join` | C→S | Join a conversation's room to receive its live events | required; DB membership re-checked | `{conversationId}` | yes — `{ok, error?}` | Not a participant → ack `{ok:false, error:{code:"FORBIDDEN"}}`, no join. Invalid id → `{ok:false, error:{code:"NOT_FOUND"}}`. |
| `conversation:leave` | C→S | Leave a room (e.g. navigated away) | required | `{conversationId}` | no | No-op if not currently joined. |
| `message:send` | C→S | Persist + broadcast a new message | required; DB membership re-checked | see §10 | **yes** — `{ok, message?, error?}` | Validation fail → ack `{ok:false, error}`. Not participant → `{ok:false, FORBIDDEN}`. Persistence fails → `{ok:false, code:"PERSIST_FAILED"}`, client may retry with same `clientMessageId` (safe, see §13). **Persistence succeeds → ack always `{ok:true, message}`, even if the subsequent broadcast step fails** — see §12a. |
| `message:new` | S→C | Deliver a newly sent message to the other participant(s) | scoped to room `conversation:<id>` (membership already enforced at join-time) | see §10 | no (fire-and-forget notification; delivery confirmed separately via `message:delivered`) | If recipient offline, or if the broadcast attempt itself throws (§12a), not received live — recovered via reconnection sync (§19) or REST history fetch either way. |
...
There is no server-side "outbox" queuing events for a disconnected socket — undelivered real-time events are simply **not** re-sent verbatim on reconnect (they were never queued in the first place; Socket.IO does not persist emitted events for offline sockets). Instead, **MongoDB is used as the recovery mechanism**: on reconnect (or on opening a conversation, or on tab focus after being backgrounded), the client fetches anything newer than the last message it has locally via `GET /conversations/:id/messages?after=<lastKnownMessageId>` (BACKEND.md §12's cursor mechanism, run in the opposite/"newer" direction). This means the *live event* and the *durable record* are different delivery paths for the same underlying data, and losing the live event is always safe because the durable path fully recovers it. See [[Reconnection]].
