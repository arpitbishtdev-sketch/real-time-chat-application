...
| `message:send` | C→S | Persist + broadcast a new message | required; DB membership re-checked | see §10 | **yes** — `{ok, message?, error?}` | **Implemented M5; rate limiting wired M10.** Over the §25 rate limit → `{ok:false, error:{code:"RATE_LIMITED"}}`, checked before validation, socket stays connected. Validation fail → ack `{ok:false, error}`. Not participant → `{ok:false, FORBIDDEN}`. Persistence fails → `{ok:false, code:"PERSIST_FAILED"}`, client may retry with same `clientMessageId` (safe, see §13). **Persistence succeeds → ack always `{ok:true, message}`, even if the subsequent broadcast step fails** — see §12a. A duplicate-`clientMessageId` retry acks `{ok:true, message}` for the original message and does not re-broadcast `message:new` (§13's "does nothing else"). |
...
| `message:send` exceeds the per-user rate limit | §25 Rate limiting (M10) |
| A single Mongoose write/read call throws | Global safety nets: `asyncHandler`+`errorHandler` (REST), handler-level try/catch+`toAckError` (sockets) — TESTING.md #24 |
| A sustained MongoDB outage, then recovery | BACKEND.md §12 bounded `serverSelectionTimeoutMS` — TESTING.md #31 |

## 25. Rate Limiting

**Implemented M10** (`sockets/rateLimit.js`, wired into `sockets/message.handlers.js`/`sockets/index.js`). `message:send` is rate-limited **per authenticated `userId`** (not per-socket, so multi-tab spam from the same user is also bounded) using a simple in-memory token bucket, one per `userId`, continuously (fractionally) refilling rather than ticking on an interval timer: capacity 20, refill 2/sec. The check runs **before** payload validation, so a flood of malformed payloads is bounded exactly as much as a flood of valid ones. Exceeding it does not disconnect the socket — the ack returns `{ok:false, error:{code:"RATE_LIMITED"}}`, and the client surfaces this distinctly from a persistence failure (retry-worthy vs. "slow down"). Built fresh per `createSocketServer()` call (same per-instance-state convention as presence/typing/message-ordering state, exposed as `io.messageRateLimiter` for tests), so multiple server instances (e.g. one per test) never share a bucket. This is the single authoritative description of this limit — BACKEND.md §10 cross-references here rather than restating the parameters.
